...
(function (global, jQuery, undefined) {
  ...
  function getHelper(helper) {
    ...
    helper = (
    view.dynCtx && view.dynCtx[helper] !== undefined ? view.dynCtx : view.ctx[helper] !== undefined ? view.ctx : tmplHelpers[helper] !== undefined ? tmplHelpers : $viewsHelpers[helper] !== undefined ? $viewsHelpers : {})[helper];
  ...
  function renderTag(tag, parentView, self, content, tagInstance) {
    ...
    var ret, linkCtx = !self.markup && self,
    ...
    tmpl = tagInstance.tmpl = "" + tmpl === tmpl // if a string
    ...
    return ret || (ret == undefined ? (tmpl ? tagInstance.renderContent(args[0], undefined, parentView) : "") : ret.toString()); // (If ret is the value 0 or false, will render to string)
  }
  //=================
  // View constructor
  //=================
  ...
  function View(context, path, parentView, data, template, key, onRender, isArray) {
    ...
    if (parentView) {
      ...
      if (parentView._useKey) {
        ...
        self.key = self.index = key !== undefined ? key : views.length, 0, self);
      }
    }
    return self;
  }
  ...
  function tmplFn(markup, tmpl, bind) {
    ...
    function parseTag(all, defer, tagName, converter, colon, html, code, params, slash, closeBlock, index) {
      ...
      if (code) {
        ...
        newNode = [
        tagName, converter || "", params, block && [], "{" + (hash ? ("props:{" + hash + "},") : "") + "data: data" + (passedCtx ? ",ctx:{" + passedCtx.slice(0, -1) + "}" : "") + "}"];
      ...
      if ("" + node === node) { // type string
        ...
        if (tag === "*") {
          ...
          if (tag.slice(-1) === "!") {
            ...
            code += (tag === ":" ? (converter === "html" ? (hasEncoder = TRUE, "h(" + params) : converter ? (hasConverter = TRUE, 'c("' + converter + '",view,this,' + params) : (getsValue = TRUE, "((v=" + params + ')!=u?v:""')) : (hasTag = TRUE, 't("' + tag + '",view,this,' + (content ? nested.length : '""') // For block tags, pass in the key (nested.length) to the nested content template
            + "," + hash + (params ? "," : "") + params)) + ")+";
          }
        }
      }
  ...
  function parseParams(params, bind, defer) {
    ...
    function parseTokens(all, lftPrn0, lftPrn, path, operator, err, eq, path2, prn, comma, lftPrn2, apos, quot, rtPrn, prn2, space) {
      ...
      if (err) {
        ...
        // path
        ? (path.replace(rPath, parsePath) + (prn ? (fnCall[++parenDepth] = TRUE, prn) : operator)) : operator ? operator : rtPrn
        // function
